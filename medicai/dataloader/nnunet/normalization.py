...
class MRINormalizer(BaseNormalizer):
    ...
    def __init__(self, nonzero_only=True, use_mask_for_norm=False):
...
def get_normalizer(scheme, stats=None, use_mask_for_norm=False):
    """
    ...
    use_mask_for_norm : if True, Z-score uses provided mask for statistics
    """
    scheme = scheme.lower()
    ...
    if scheme == "ct":
        ...
        return cls(stats)
    if scheme == "z_score":
        return cls(use_mask_for_norm=use_mask_for_norm)
    return cls()
