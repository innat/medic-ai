...
BACKBONE_ARGS = {
    'efficientnetb0': [
        'block6a_expand_activation', 'block4a_expand_activation','block3a_expand_activation', 'block2a_expand_activation'
    ],
    
    'resnet50': [
        'conv4_block6_2_relu', 'conv3_block4_2_relu', 'conv2_block3_2_relu', 'conv1_relu'
    ],
...
def Conv3x3BNReLU(filters):
    def apply(input):
        x = nn.Conv2D(
            filters, kernel_size=(3,3), activation='relu', padding='same', kernel_initializer='he_normal'
        )(input)
