...
def transform_specs(layout: str, spatial_size: int) -> list[BenchmarkSpec]:
    ...
    specs = [
        ...
        BenchmarkSpec(
            "RandomFlip",
            "cpu+gpu",
            lambda l, s: RandomFlip(
                keys=["image", "label"], spatial_axis=axis, prob=1.0, seed=s, input_layout=l
            ),
            True,
        ),
        BenchmarkSpec(
            "RandomRotate90",
            "cpu+gpu",
            lambda l, s: RandomRotate90(
                keys=["image", "label"], max_k=3, prob=1.0, seed=s, input_layout=l
            ),
            True,
        ),
        BenchmarkSpec(
            "RandomRotate",
            "cpu+gpu",
            lambda l, s: RandomRotate(
                keys=["image", "label"], factor=0.1, prob=1.0, seed=s, input_layout=l
            ),
            True,
        ),
        BenchmarkSpec(
            "RandomShiftIntensity",
            "cpu+gpu",
            lambda l, s: RandomShiftIntensity(
                keys=["image"], offset=0.1, prob=1.0, seed=s, input_layout=l
            ),
            True,
        ),
        BenchmarkSpec(
            "RandomSpatialCrop",
            "cpu+gpu",
            lambda l, s: RandomSpatialCrop(
                keys=["image", "label"], crop_size=crop_shape, input_layout=l, seed=s
            ),
            True,
        ),
        BenchmarkSpec(
            "RandomCutOut",
            "cpu+gpu",
            lambda l, s: RandomCutOut(
                keys=["image"], mask_size=(4, 4), num_cuts=1, prob=1.0, input_layout=l, seed=s
            ),
        ),
    ]
        ...
        specs.extend(
            [
                ...
                BenchmarkSpec(
                    "Orientation",
                    "cpu",
                    lambda l, s: Orientation(
                        keys=["image", "label"], axcodes="RAS", input_layout=l
                    ),
                    True,
                ),
                BenchmarkSpec(
                    ...
                    "cpu",
                    lambda l, s: Spacing(
                        keys=["image", "label"], pixdim=(2.0, 2.0, 2.0), input_layout=l
                    ),
                    True,
                ),
            ]
